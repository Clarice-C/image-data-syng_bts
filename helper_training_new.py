...
def training_AEs(savepath,             # path to save reconstructed samples
                 ...
                 feat_cols=None):      # whether to plot the heatmaps of reconstructed and newly generated samples with the original ones
    ...
    if modelname == "CVAE" or modelname == "MultiCVAE":
        ...
        if early_stop:
            if save_recons:
                # Plot generated data
                plot_recons_samples(savepath = savepath, data_loader = train_loader, model = best_model, n_features = num_features, modelname = modelname, plot = plot)       
                plt.show()
            if save_new:
                # plot and save new generated data
                plot_new_samples(model = best_model, savepathnew = savepathnew, latent_size = 32, modelname = modelname, num_images = new_size, plot = plot, colnames = colnames,
                    preprocess = preprocess, preprocess_params_path = preprocess_params_path, preprocess_params_df = preprocess_params_df, feat_cols = feat_cols)
                plt.show()
        else:
            if save_recons:
                # Plot generated data
                plot_recons_samples(savepath = savepath, data_loader = train_loader, model = model, n_features = num_features, modelname = modelname, plot = plot)       
                plt.show()
            if save_new:
                # plot and save new generated data
                plot_new_samples(model = model, savepathnew = savepathnew, latent_size = 32, modelname = modelname, num_images = new_size, plot = plot, colnames = colnames,
                    preprocess = preprocess, preprocess_params_path = preprocess_params_path, preprocess_params_df = preprocess_params_df, feat_cols = feat_cols)
                plt.show()
    elif modelname=="VAE":
