...
class MultiCVAE(nn.Module):
    """
    Multi-conditional CVAE supporting multiple conditional variables.
    For groups (0/1) and groups2 (0/1/2), the condition vector is:
    y = [onehot(groups), onehot(groups2)] with length 5
    """
    def __init__(self, num_features, condition_dim):
        ...
        # encoder input: features + condition vector
        self.encoder = nn.Sequential(
            nn.Linear(num_features + condition_dim, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(True),
            nn.Linear(256, 128),
            nn.BatchNorm1d(128),
        ...
        # decoder input: latent + condition vector
        self.decoder = nn.Sequential(
            nn.Linear(32 + condition_dim, 64),
            nn.BatchNorm1d(64),
            nn.ReLU(True),
            nn.Linear(64, 128),
            nn.BatchNorm1d(128),
